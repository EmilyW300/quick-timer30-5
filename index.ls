...
update-pause-count = ->
  count = state.pause-count
  document.getElementById('pause-count').textContent = count
  icon-count = document.getElementById 'pause-count-icon'
  if icon-count => icon-count.textContent = count
...
adjust-time = (delta-seconds) ->
  new-remaining = Math.max 0, state.remaining + delta-seconds
  state.remaining := new-remaining
  update-display true
  if new-remaining == 0
    finish!
  else
...
setup = ->
  ...
  document.getElementById('minus-3-btn').addEventListener 'click', -> adjust-time -180
  document.getElementById('minus-5-btn').addEventListener 'click', -> adjust-time -300
  document.getElementById('minus-10-btn').addEventListener 'click', -> adjust-time -600
  update-display true
  update-pause-count!
  update-pause-button!
